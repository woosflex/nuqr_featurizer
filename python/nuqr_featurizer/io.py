...
def _get_pillow_image():
    try:
        ...
        raise ImportError(
            'Pillow is required for file-based APIs. Install with `pip install "nuqr-featurizer[io]"`.'
        ) from exc
    return Image
...
def _get_scipy_loadmat():
    try:
        ...
        raise ImportError(
            'scipy is required for .mat loading. Install with `pip install "nuqr-featurizer[io]"`.'
        ) from exc
    return loadmat
